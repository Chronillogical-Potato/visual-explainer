...
Write to `~/.agent/diagrams/` with a descriptive filename, make it self-contained (render tool, or `offline/inline-assets.mjs` — see SKILL.md "Cookie offline delivery"), and open the result in the browser. In Pi package installs, this is an explicit visual request: use `visual_explainer.prepare` when planning/context scouting helps, then `visual_explainer.render` with the complete HTML.
